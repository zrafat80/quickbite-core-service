// src/address/repository/address.repository.ts
...
export class AddressRepository {
  ...
  async createAddress(userId: number, data: CreateAddressDTO): Promise<Address> {
  ...
  async getAddressByIdAndUserId(addressId: number, userId: number): Promise<Address | undefined> {
    const row = await this.knex('customer_addresses')
      .select(ADDRESS_COLUMNS) // <-- Safe SELECT!
  ...
  async updateAddress(addressId: number, userId: number, data: any): Promise<boolean> {
    ...
    if (data.apartmentNumber !== undefined) updatePayload.apartment_number = data.apartmentNumber;
